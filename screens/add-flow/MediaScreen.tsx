...
const MediaScreen = observer(
  ({ navigation, route }: ScreenProps) => {
    ...
    const defaultImagesList = route.params.method === "add" ? [] :
      editFlowStore.currentRecordAttachments.map(item => ({ uri: item.uri, type: item.type }));
        ...
        <AddFlowNavBar
          ...
          right={async () => {
            if (route.params.method === "add") {
              ...
              const savedPaths = await Promise.all(paths.map(path => moveToFileSystem(path)));
              const finalImages = images.map((item, index) => ({ type: item.type, uri: savedPaths[index]}))
