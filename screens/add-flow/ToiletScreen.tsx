...
const ToiletScreen = observer(
  ({ navigation, route }: ScreenProps) => {
    const { addFlowStore, progressStore, editFlowStore } = useStores();
    const defaultPee = route.params.method === "add" ? null :
      !editFlowStore.currentEditingRecord ? null :
      editFlowStore.currentEditingRecord.toiletType === 0 ? true :
      editFlowStore.currentEditingRecord.toiletType === 2 ? true : false;
    const defaultPoo = route.params.method === "add" ? null :
    !editFlowStore.currentEditingRecord ? null :
    editFlowStore.currentEditingRecord.toiletType === 1 ? true :
    editFlowStore.currentEditingRecord.toiletType === 2 ? true : false;
